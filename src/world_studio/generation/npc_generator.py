...
class NpcGenerator:
    def generate(self, social_service: object, context: GenerationContext) -> None:
        for settlement_ref in context.settlement_refs:
            ...
            settlement_modifier = context.modifiers.settlement.get(settlement_ref)
            ...
            if settlement_modifier is not None:
                target = int(target * settlement_modifier.population_multiplier())
                target = max(min_npcs, target)
                min_npcs = max(min_npcs, int(min_npcs * settlement_modifier.population_multiplier()))
                max_npcs = max(min_npcs, int(max_npcs * settlement_modifier.population_multiplier()))
            ...
            for idx in range(target):
                ...
                if settlement_modifier is not None and settlement_modifier.age_shift_years != 0:
                    age_years = max(16, age_years + settlement_modifier.age_shift_years)
                notes = (
                    ...
                    f"Theme={context.settlement_themes.get(settlement_ref, 'mixed')}. "
                    f"Tension={context.settlement_tensions.get(settlement_ref, 'stable')}."
                )
                metadata = {
                    "generation_theme": context.settlement_themes.get(settlement_ref, "mixed"),
                    "generation_tension": context.settlement_tensions.get(settlement_ref, "stable"),
                    "generation_tags": context.settlement_tags.get(settlement_ref, []),
                }
